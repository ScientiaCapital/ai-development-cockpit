 ...
 *
 * FALLBACK: When rate-limited (HTTP 429), returns pre-seeded REAL data
 * from /coperniq-cache.json (generated by seed script from local API calls).
 */
...
/**
 * Load pre-seeded cache from static file (REAL data, not mock)
 * This is generated by running: npm run seed:coperniq locally
 */
async function loadPreSeededCache(baseUrl: string): Promise<Record<string, unknown> | null> {
  try {
    const cacheUrl = `${baseUrl}/coperniq-cache.json`;
    const res = await fetch(cacheUrl);
    if (!res.ok) return null;

    const cache = await res.json();
    console.log('[Coperniq All] Using pre-seeded cache from', cache.seededAt);
    ...
    return {
      ...
      seededAt: cache.seededAt,
      customers: cache.rawClients?.map(transformClient) || [],
      customersCount: cache.clientsCount || 0,
      projects: cache.rawProjects?.map(transformProject) || [],
      projectsCount: cache.projectsCount || 0,
      requests: [],
      requestsCount: 0,
      workOrders: cache.rawProjects?.map(transformProjectToWorkOrder) || [],
      workOrdersCount: cache.projectsCount || 0,
      invoices: [],
      invoicesCount: 0,
    };
  } catch (error) {
    console.error('[Coperniq All] Failed to load pre-seeded cache:', error);
    return null;
  }
}

// Transform functions for pre-seeded data (raw Coperniq format)
function transformClient(client: Record<string, unknown>): Record<string, unknown> {
  return {
    id: `client-${client.id}`,
    name: client.title || client.name || 'Unknown',
    email: client.primaryEmail,
    phone: client.primaryPhone,
    address: formatAddressFromRaw(client),
...
function transformProject(proj: Record<string, unknown>): Record<string, unknown> {
  const client = proj.client as Record<string, unknown> | undefined;
  return {
    id: `proj-${proj.id}`,
    title: proj.title || 'Untitled Project',
    customer: client?.name || 'Unassigned',
    address: formatAddressFromRaw(proj),
    stage: 'lead',
...
function transformProjectToWorkOrder(proj: Record<string, unknown>): Record<string, unknown> {
  const client = proj.client as Record<string, unknown> | undefined;
  return {
    id: `proj-${proj.id}`,
    title: proj.title || 'Untitled Project',
...
function formatAddressFromRaw(obj: Record<string, unknown>): string {
  const parts: string[] = [];
  ...
  if (obj.zipcode) parts.push(obj.zipcode as string);
  return parts.join(', ');
}
...
export async function GET(request: NextRequest) {
  ...
  if (!apiKey) {
    ...
    // Try pre-seeded cache even without API key
    const cached = await loadPreSeededCache(baseUrl);
    if (cached) {
  ...
  // FALLBACK: If rate-limited on all endpoints, use pre-seeded REAL data
  if (rateLimited && (results.customersCount === 0 || results.customersCount === undefined)) {
    console.log('[Coperniq All] Rate limited - falling back to pre-seeded cache');
    const cached = await loadPreSeededCache(baseUrl);
    if (cached) {
      const response = NextResponse.json(cached);
      response.headers.set('Cache-Control', `public, s-maxage=${CACHE_TTL}, stale-while-revalidate=${CACHE_TTL * 2}`);
    ...
    console.warn('[Coperniq All] No pre-seeded cache available');
  }
